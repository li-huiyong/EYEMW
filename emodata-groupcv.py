"""
emodata：仅 7 维眼动特征，按 ParticipantNum 分组的嵌套交叉验证（外层 K 折、内层调参），
同一受试者不会同时出现在训练折与测试折。Pipeline：Imputer → StandardScaler → SVC(RBF)。
"""
import warnings
...
def main():
    ...
    print(
        f"特征: 7 维眼动 | 外层 GroupKFold n_splits={OUTER_SPLITS}，"
        f"内层 GroupKFold n_splits={INNER_SPLITS} | 内层评分: f1_macro"
    )
    ...
    print("\n===============================================")
    print(f"{OUTER_SPLITS}-Fold GroupCV Summary (emodata, 7 gaze + SVM)")
