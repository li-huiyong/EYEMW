"""
eyedata：按受试者分组的嵌套交叉验证（外层 K 折、内层调参），保证同一 ParticipantNum
不会同时出现在训练折与测试折。Pipeline：Imputer → StandardScaler → SVC(RBF)。
"""
import warnings
...
def main():
    ...
    print("\n===============================================")
    print(f"{OUTER_SPLITS}-Fold GroupCV Summary (eyedata, 7 gaze + SVM)")
