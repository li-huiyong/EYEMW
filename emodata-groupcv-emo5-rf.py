"""
emodata：3 情感 + AOIGazeProportion + OffScreenGazeProportion（5 维），随机森林分类。
外层 / 内层均为 GroupKFold(groups=ParticipantNum)。Pipeline：Imputer → RandomForestClassifier
（树模型不对特征做 StandardScaler）。内层 GridSearchCV 以 f1_macro 调参；外层测试折仅评估。
"""
import warnings
...
def main():
    ...
    print(
        f"模型: RandomForestClassifier | 特征: 5 维（3 情感 + 2 注视比例）| "
    ...
    print("\n===============================================")
    print(f"{OUTER_SPLITS}-Fold GroupCV Summary (emodata, 5 features + RandomForest)")
