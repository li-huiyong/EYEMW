"""
emodata：按被试固定三分 train:val:test = 7:1.5:1.5（被试人数约 70% / 15% / 15%），
仅用 7 维眼动特征。同一被试只落在一个子集；预处理仅在训练行上 fit；
验证集上网格选 macro-F1，train+val 重训，测试集仅评一次。
"""
import warnings
...
def split_participants_715(pids: np.ndarray, y_strat: np.ndarray, random_state: int = 42):
    test_size = 0.15 / (0.7 + 0.15 + 0.15)
    val_of_trainval = 0.15 / (0.7 + 0.15)
...
def main():
    ...
    print("数据:", DATA_PATH, "| 特征: 7 维眼动（与 eyedata 列名一致）")
    print(
